...
if($logged_in)
{
	...
	if(isset($_POST['save']))
	{
		...
		{
			if (strlen($v) > 0 && strpos($v, "http://") === false && strpos($v, "https://") === false)
			{
				$links[$k] = "http://". $v;
			}
		}
				...
				if ( ! preg_match($pattern, $links[$field]))
				{
					switch ($field)
					{
						...
						case 'twitter':
							...
							$errors[$field] = $lang['profile_social_twitter'] . ': '. $lang['profile_msg_social_link'];
							
						break;
						
						case 'lastfm':
							
							$errors[$field] = $lang['profile_social_lastfm'] . ': '. $lang['profile_msg_social_link'];
