			...
			if(isset($_POST['Submit']))
			{
				...
				else
				{
					...
					else
					{
						...
						$msg = 'The video ad was successfully created.';
						if($status == 0)
	...
	case 'edit':
		...
		else
		{
			if(isset($_POST['Submit']))
			{
				...
				{
					$info_msg = pm_alert_success('<strong>'. $name .'</strong> updated.');
	...
	default:
		...
		if($action == 'reset')
		{
			...
			else
			{
				...
				if ( ! $query )
				{
					$info_msg = pm_alert_error('A problem was encountered while performing your request.<br />MySQL returned: '. mysql_error());
				}
			}
		}
	?>
