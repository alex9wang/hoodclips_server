...
function get_thumb_link($video_id) 
{
	...
	$target_url = 'http://www.cloudy.ec/v/'. $video_id;
...
function video_details($video_data, $url, &$info) 
{
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		if ( ! $title)
		{
			if (strpos($video_data[$i], 'og:title'))
			{
				...
				{
					$info['video_title'] = str_replace('|  cloudy.ec Videos', '', $matches[1]);
					$title = true;
					$title = true;
				}
			}
		...
		if ( ! $id)
		{
			...
			{
				if (preg_match('/embed.php?id="(.*?)"/', $video_data[$i], $matches) != 0)
				{
	...
	if ($info['direct'] == '')
	{
		$info['direct'] = 'https://www.cloudy.ec/embed.php?id='. $info['yt_id'];
	}
