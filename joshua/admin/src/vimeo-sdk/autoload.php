...
//		$args['query'] = urlencode($keywords);
	...
	public function pm_user_playlists($username, $args = array()) 
	{
		...
		foreach ($result['body']['data'] as $k => $item)
		{
			...
			$tmp['url'] = $item['link'];
			$tmp['total_thumbs'] = 1;
			$tmp['thumbs'] = array(0 => array('original' => $item['pictures']['sizes'][ count($item['pictures']['sizes']) - 1 ]['link'],
												'small' => $item['pictures']['sizes'][0]['link'],
												'medium' => $item['pictures']['sizes'][1]['link'],
												'large' => $item['pictures']['sizes'][3]['link'],
										)
								);
	...
	public function pm_get_user_avatar_url($username)
	{
		...
		if ($result['status'] == 200)
		{
			foreach ($result['body']['data'][0]['sizes'] as $k => $img)
			{
				if ($img['width'] >= 80)
				{
					return str_replace('https:', 'http:', $img['link']);
				}
			}
		}
		
		return 'http://i.vimeocdn.com/portrait/default_75x75.jpg';
	}
