...
function get_thumb_link($video_id) 
{
	...
	$target_url = 'http://www.myvideo.ge/?video_id='. $video_id;
	$video_data = get_info($target_url);
	$arr_length = count($video_data);

	for($i = 0; $i < $arr_length; $i++)
	{
		if(preg_match('/src=\"(.*?)\"(.*?)\/>/i', $video_data[$i], $matches) != 0)
		{
...
function video_details($video_data, $url, &$info) 
{
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		if ( ! $id)
		{
			...
			{
//				if (preg_match('/?video_id="(.*?)"/', $video_data[$i], $matches) != 0)
				if (preg_match('/content="(.*?)"/', $video_data[$i], $matches) != 0)

				{
					$pieces = explode('?video_id=', $matches[1]);
					$info['yt_id'] = $pieces[1];// trim( $pieces[count($pieces) - 1]);
					$yt_id = true;
				}
			}
		}
	...
	if ($info['direct'] == '')
	{
		$info['direct'] = 'http://embed.myvideo.ge/flv_player/player.php?video_id='. $info['yt_id'] .'.mp4';
	}
