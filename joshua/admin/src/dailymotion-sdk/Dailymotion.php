...
class PhpmelodyDailymotion extends Dailymotion
{
	...
	public $video_object_fields = array('id', 'title', 'description', 'duration', 
										'thumbnail_url', 'thumbnail_60_url', 'thumbnail_120_url', 'thumbnail_240_url', 'thumbnail_360_url', 'thumbnail_720_url',
										'url', 'embed_url', 'tags', 'explicit',
										'access_error', 'allow_embed', 'geoblocking', 'mediablocking', 'paywall', 'private', 'svod', 'tvod',
	...
	public function pm_user_playlists($username, $args = array()) 
	{
		...
		$result = $this->get('/user/'. urlencode($username) .'/playlists' .'?page='. $args['page'] .'&limit='. $args['per_page'] . $filters,
							array('fields' => array('id', 'name', 'description', 'owner.avatar_120_url', 'thumbnail_url', 'thumbnail_60_url', 'thumbnail_120_url', 'thumbnail_240_url', 'videos_total'))
					);
		...
		foreach ($result['list'] as $k => $item)
		{
			...
			$tmp['total_thumbs'] = 1;
			$tmp['thumbs'] = array(0 => array('original' => $item['thumbnail_url'],
												'small' => $item['thumbnail_120_url'],
												'medium' => $item['thumbnail_240_url'],
												'large' => $item['thumbnail_720_url'],
												/*'60' => $item['thumbnail_60_url'],
												'120' => $item['thumbnail_120_url'],
												'360' => $item['thumbnail_360_url']*/
											)
								);
	...
	public function pm_get_user_avatar_url($username)
	{
		try {
			...
			{
				return $result['avatar_80_url'];
			}

		} catch(DailymotionApiException $e) {
			return 'http://s1.dmcdn.net/default.png';
		}
