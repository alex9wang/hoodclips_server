<?php
require_once ABSPATH . '/admin/src/youtube-sdk/src/Google/autoload.php';
require_once ABSPATH . '/admin/src/youtube-sdk/src/Google/Service/YouTube.php';
...
class PhpmelodyYoutube extends Google_Service_YouTube
{
	 ...
	 * 
	 * @param string $yt_id the youtube video id; pass just one id
	 * @return int the determined value of VS_* constant
	 */
	public function pm_get_video_status($yt_id = '')
	{
		...
		// it's cheaper if we split it into two requests but the question is, which info to request first?
		// check status property first, presumably the most common cause of concern
		try {
			$result = $this->videos->listVideos('status', array('id' => $yt_id));

			if ($result['pageInfo']['totalResults'] == 0
				|| $result['items'][0]['status']['uploadStatus'] == 'deleted'
				|| $result['items'][0]['status']['embeddable'] == false 
				|| $result['items'][0]['status']['privacyStatus'] == 'private')
			{
				return VS_BROKEN;
			}
		...
		// check regionRestriction
		try {
			$result = $this->videos->listVideos('contentDetails', array('id' => $yt_id));

			if (is_object($result['items'][0]['contentDetails']['regionRestriction']))
			{
				return VS_RESTRICTED;
			}
	...
	public function pm_user_playlists($username, $args = array())
	{
		...
		if ( ! $this->pm_channel_id)
		{
			try {
				...
				if ($result['pageInfo']['totalResults'] == 0)
				{
					return array('error' => array('message' => '<strong>User not found</strong>'));
				}
		...
		try {
			$result = $this->playlists->listPlaylists('id,snippet', array('channelId' => $this->pm_channel_id, 'maxResults' => 50));
			...
			foreach ($result['items'] as $item)
			{
				...
				$tmp['total_thumbs'] = 1;
				$tmp['thumbs'] = array(0 => array('original' => $item['snippet']['thumbnails']['high']['url'],
				...
				foreach ($tmp['thumbs'][0] as $kk => $thumb_url)
				{
					$tmp['thumbs'][0][$kk] = str_replace('https:', 'http:', $thumb_url);
				}
	...
	public function pm_format_video_data($result, $detailed_video_data = true)
	{
		...
		foreach ($result['items'] as $item)
		{
			...
			$tmp['thumbs'] = array( 0 => array('original' => $item['snippet']['thumbnails']['high']['url'],
									...
									1 => array('original' => 'http://img.youtube.com/vi/'. $id .'/2.jpg',
												'small' => 'http://img.youtube.com/vi/'. $id .'/2.jpg',
												'medium' => 'http://img.youtube.com/vi/'. $id .'/2.jpg',
												'large' => 'http://img.youtube.com/vi/'. $id .'/2.jpg',
										),
									2 => array('original' => 'http://img.youtube.com/vi/'. $id .'/3.jpg',
												'small' => 'http://img.youtube.com/vi/'. $id .'/3.jpg',
												'medium' => 'http://img.youtube.com/vi/'. $id .'/3.jpg',
												'large' => 'http://img.youtube.com/vi/'. $id .'/3.jpg',
										),
			);
			...
			$tmp['keywords'] = '';
			$tmp['url'] = 'http://www.youtube.com/watch?v='. $id;
			$tmp['embed_url'] = '//www.youtube.com/v/'. $id .'?&autoplay=1&v='. $id .'&version=3';
			$tmp['url_flv'] = '';
			$tmp['mp4'] = '';
	...
	public function pm_get_user_avatar_url($username)
	{
		...
		try {
			...
			return str_replace('https:', 'http:', $result['items'][0]['snippet']['thumbnails']['default']['url']);
