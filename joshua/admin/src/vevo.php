...
function get_thumb_link($video_id) 
{
	...
	$target_url = 'http://www.vevo.com/watch/'. $video_id;
...
function video_details($video_data, $url, &$info) 
{
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		{
			$temp_url = '';
			if (strpos($video_data[$i], '"twitter:app:url:iphone"'))
			{
				if (preg_match('/twitter:app:url:iphone" content="(.*?)"/', $video_data[$i], $matches) != 0)
				{
	...
	if ($info['direct'] == '')
	{
		$info['direct'] = 'http://www.vevo.com/watch/'. $info['yt_id'];
	}
