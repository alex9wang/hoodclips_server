...
$page	= (int) $_GET['page'];
$filter = 'added';
$filters = array('broken', 'restricted', 'unchecked', 'localhost', 'featured', 'category', 'source', 'mostviewed', 'access', 'views', 'added', 'addedactive');
$filter_value = 'desc';
...
{
	echo pm_alert_success('Video(s) successfully removed.');
}

if ($_GET['action'] == 'deletedcomments') 
{
	echo pm_alert_success('Comment(s) successfully removed.');
}
	...
	if($total_ids > 0)
	{
		...
		if(strlen($in_arr) > 0)
		{
			...
			else
			{
				...
				foreach ($video_list_data as $uniq_id => $v)
				{
					$video_id = $v['id'];
				...
				foreach ($video_list_data as $uniq_id => $row)
				{
					...
					if ($result = @mysql_query($sql))
					{
						...
						{
							playlist_delete_item($list['list_id'], $row['id']);
						}
						mysql_free_result($result);
					}
				}
				...
				echo pm_alert_success('Video(s) removed successfully.');
			}
	...
	else
	{
		...
		else
		{
			...
			{				
				...
				echo pm_alert_success('Video(s) successfully moved to <strong>'. $categories[$new_cid]['name'].'</strong>.');
...
if ($_POST['Submit'] == 'Restrict access' || $_POST['Submit'] == 'Derestrict access')
{
	...
	if ($total_ids > 0)
	{
		...
		{
			echo pm_alert_success('Video(s) updated successfully.');
			$videos = a_list_videos('', '', $from, $limit, $page, $filter, $filter_value); 
		}
	}
	else
			...
			else
			{
				...
				*/// @deprecated since v2.2
				...
				$sql = "DELETE FROM pm_playlists 
						WHERE type NOT IN (". PLAYLIST_TYPE_WATCH_LATER .", ". PLAYLIST_TYPE_FAVORITES .", ". PLAYLIST_TYPE_LIKED .")";
				$result = @mysql_query($sql);
				if(!$result)
					echo pm_alert_error('There was an error while updating your database.<br />MySQL returned: '. mysql_error());
				...
				$sql = "UPDATE pm_playlists 
						SET items_count = 0 
						WHERE type IN (". PLAYLIST_TYPE_WATCH_LATER .", ". PLAYLIST_TYPE_FAVORITES .", ". PLAYLIST_TYPE_LIKED .")";
				$result = @mysql_query($sql);
				if(!$result)
					echo pm_alert_error('There was an error while updating your database.<br />MySQL returned: '. mysql_error());
...
<div class="btn-toolbar">
    ...
    <div class="btn-group dropup">
        ...
        <ul class="dropdown-menu pull-right">
            <li><a href="videos.php?action=9" rel="tooltip" title="This will remove ALL existing videos and their comments.">Delete all videos</a></li>
        </ul>
    </div>
