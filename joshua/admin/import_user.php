...
if(($action == 'search' || $action == 'favorites' || $action == 'playlists') && ($username != ''))
{
	...
	switch ($data_source)
	{
		case 'youtube':
			...
			$playlists = $youtube_api->pm_user_playlists($username);
  ...
  <div id="import_user">
	...
	switch($action)
	{
		...
		case 'favorites':
			...
			?><h2 class='sub-heading'><strong><?php echo ucfirst($username) ?>'s Favorite Videos</strong></h2><?php
...
switch ($action)
{
	...
	case 'playlists':
		...
		$sub_name = $username .'/'. $playlist_title;
		$sub_params = array(
							'action' => $action,
							'username' => $username,
							'results' => $import_results,
