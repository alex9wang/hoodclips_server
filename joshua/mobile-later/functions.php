...
function getavatar($user_id, $username) {
	...
	if ($user_id == 0)
	{
		$avatar_url = _URL .'/'. _UPFOLDER .'/avatars/pm-avatar.png';
	}
	else
	{
		...
		else
		{
			...
			{
				$avatar_url = _URL .'/'. _UPFOLDER .'/avatars/'. $user['avatar'];
			}
		}
...
if($date_diff < _ISNEW_DAYS)
	$listing .= '<span class="is_new">NEW</span>';
if($video['site_views'] > _ISPOPULAR)
	$listing .= '<span class="is_hot">HOT</span>';
if($video['featured'] == 1)
	$listing .= '<span class="is_featured">FEATURED</span>';
	
	$listing .= '<div class="duration">'.sec2min_mobi($video['yt_length']).'</div>
					<a href="'.makevideolink_mobi($video['uniq_id'], $video['video_title'], $video['video_slug']).'"><img src="'.show_thumb($video['uniq_id']).'" border="0" class="border2 border_th" /></a>
				</div>
				<div id="video_nfo">
					...
					<div class="added_date">'.time_since($video['added']).' '.$lang['ago'].'<br/>'.$video['site_views'].'views</div>
				</div>
				</div>
			</li>';
...
function request_video_mobi($unique_id = '', $page = "detail", $autoplay = false)
{
	global $config;
...
function generate_player($uniq_id, $width = '', $height = '')
{
	...
	$video = request_video_mobi($uniq_id, 'embed', true);
...
*/
		...
		case '3':
		$embed_code = '<iframe width="'.$width.'" height="'.$height.'" src="http://www.youtube.com/embed/'.$video['yt_id'].'?rel=0&amp;hd=1" frameborder="0" allowfullscreen></iframe>';
		break;
		...
		case '5':
		$embed_code = '<iframe frameborder="0" width="'.$width.'" height="'.$height.'" src="http://www.dailymotion.com/embed/video/'.$video['yt_id'].'"></iframe>';
		break;
		
		case '16':
		$embed_code = '<iframe src="http://player.vimeo.com/video/'.$video['yt_id'].'?byline=0&amp;portrait=0&amp;player=mobile" width="'.$width.'" height="'.$height.'" frameborder="0" webkitAllowFullScreen allowFullScreen></iframe>';
		break;

		case '18':
		$embed_code = '<iframe src="http://blip.tv/play/'.$video['yt_id'].'.html" width="'.$width.'" height="'.$height.'" frameborder="0" allowfullscreen></iframe>';
		break;
...
function makevideolink_mobi($uniq_id, $video_title = '', $video_slug = '')
{
	...
	if (_SEOMOD == 1) 
	{
		...
		{
			$r['video_title'] = $video_title .'-video';
		}
		else
		{
