    ...
    <div class="container-fluid">
      <div class="row-fluid">
        ...
        <div class="pm-article-info">
            <strong>{$lang.articles_published}</strong>: <time class="entry-date" datetime="{$article.html5_datetime}" title="{$article.full_datetime}" pubdate>{$article.date}</time> {$lang.articles_by} <a href="{$smarty.const._URL}/profile.{$smarty.const._FEXT}?u={$article.username}" itemprop="author">{$article.name}</a> 
        ...
        <div class="row-fluid pm-author-data pm-article-author">       
            ...
            <div class="span10">
                <div class="pm-submit-data"><a href="{$smarty.const._URL}/profile.{$smarty.const._FEXT}?u={$article.author_username}">{$article.name}</a></div>
                ...
                {if $smarty.const._MOD_SOCIAL && $logged_in == '1' && $article.author != $s_user_id}
                    {include file="user-follow-button.tpl" profile_data=$article profile_user_id=$article.author}
                {/if}
            </div>
        </div><!--.pm-author-data-->
		...
		{else}
			<span id="comment-list-container">
			    {include file="comment-list.tpl" tpl_name="article-read"}
