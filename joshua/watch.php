...
header("Location: http://www.hoodclips.com/watch.php?vid=".$video_uniq_id);
die();
session_start();
require('config.php');
...
if (( ! $video) || ( ! is_admin() && ( ! is_moderator() || (is_moderator() && mod_cannot('manage_videos'))) && $video['added_timestamp'] > time()))
{
	header('Location: '. _URL .'/index.'. _FEXT);
	exit();
}
...
include("footer.php");
$t_videos = top_videos("views", 10);

//map embed code from pm_embeb table//
if(!empty($t_videos) && sizeof($t_videos) > 0){
    for($i = 0;$i < sizeof($t_videos); $i++){
         $get_url = mysql_fetch_array(mysql_query("SELECT * FROM `pm_embed_code` WHERE uniq_id = '".$t_videos[$i]['uniq_id']."'"));
         $embed_links[$t_videos[$i]['uniq_id']] = $get_url['embed_code'];
    }
}
$get_url_vid = mysql_fetch_array(mysql_query("SELECT * FROM `pm_embed_code` WHERE uniq_id = '".$video_uniq_id."'"));
$video_url = $get_url_vid['embed_code'];
$smarty->assign('video_url_get', $video_url);
$smarty->assign('video_unique_id',$video_uniq_id);
$smarty->assign('video_data', $video);
$smarty->assign('date_added',date("M d, Y",strtotime($video['date_added'])));
$smarty->assign('top_video_data',$t_videos);
$smarty->assign('top_video_embed_data',$embed_links);
