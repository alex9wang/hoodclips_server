	...
	<div class="span12">
      ...
      '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" rel="tooltip" title="Share on Google+"><i class="pm-vc-sprite google-icon"></i></a>  
			...
			<div class="row-fluid pm-author-data">
                ...
                <div class="span7">
                	<div class="pm-submit-data">{$lang.added} <time datetime="{$video_data.html5_datetime}" title="{$video_data.full_datetime}">{$video_data.time_since_added} {$lang.ago}</time> {$lang._by} <a href="{$smarty.const._URL}/profile.{$smarty.const._FEXT}?u={$video_data.author_username}">{$video_data.author_name}</a> {$lang._in} {$category_name}</div>   
