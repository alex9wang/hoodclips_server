	...
	case 'video':
		...
		switch ($action)
		{
			...
			case 'report':
				...
				{
					$error_message = explode(':', $_GET['error-message']);
					switch ($error_message[0])
					{
						case 'Error loading player':
						case 'Error loading skin':
						case 'Error loading playlist':
						case 'Error drawing fallback':
							exit();
						break;
					}
	...
	case 'playlists':
		...
		switch ($action)
		{
			...
			case 'update-playlist':
				...
				if ($playlist['type'] != PLAYLIST_TYPE_WATCH_LATER && $playlist['type'] != PLAYLIST_TYPE_FAVORITES && $playlist['type'] != PLAYLIST_TYPE_LIKED)
				{
