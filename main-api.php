	...
	if($api_method == "UploadVideo"){
		...
		if(file_exists($name_file_baru)){
			...
			if(move_uploaded_file($tmp_name, $name_file_baru) && move_uploaded_file($tmp_name_thumb, $name_file_baru_thumb)){
			 ...
			 //www.hoodclips.com
				$sql = "insert into uploaded_videos(video_url, video_title, video_desc, site_views, featured, yt_thumb, date_added, email_address) values('http://www.hoodclips.com/uploaded_video/".basename($name_file)."','".$videoTitle."','".$videoDesc."', 0, 0, 'http://www.hoodclips.com/uploaded_image/".basename($name_file_thumb)."', '".date("Y-m-d")."', '".$videoEmail."');";
		...
		$sql = "select * from uploaded_videos order by id limit ".$startIndex.",".$pageLimit.";";	
