					...
					if (count($latest_videos) > 0)
					{
						foreach ($latest_videos as $time => $video)
						{
							...
							<li>
								...
								<div class="item-img">
									<a href="<?php echo $video_link;?>" target="_blank"><img src="<?php echo $video['thumb_url']; ?>" width="60" height="30" /></a>
								</div>
								<div class="item-details">
									<a href="<?php echo $video_link;?>" target="_blank" class="item-url" title=""<?php echo $video_link;?>"><?php echo $video['video_title']; ?></a>
								</div>
					...
					if ($result = mysql_query($sql))
					{
						while ($row = mysql_fetch_assoc($result))
						{
							...
							<li>
								...
								<div class="item-details"><a href="<?php echo _URL .'/profile.php?u='. $row['username']; ?>" target="_blank" class="item-user-url"><?php echo $row['username']; ?></a> from <?php echo countryid2name($row['country']); ?></div>
								<div class="item-actions">
				...
				<ul class="unstyled plist-comments">
					...
					if ($result = mysql_query($sql))
					{
						...
						foreach ($comments_data as $k => $c)
						{
							...
							if ($c['user_id'] != 0)
							{
								$profile_url = _URL .'/profile.php?u='. $row['username'];
							}
							else 
							{
								$profile_url = '#';
