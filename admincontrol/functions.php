...
function a_list_videos($search_term, $search_type = 'video_title', $from = 0, $to = 20, $page = 1, $filter = "", $filter_value = "") 
{
	...
	if($count >= 1) 
	{
		...
		while($r = mysql_fetch_array($query)) 
		{
			...
			if ($r['added'] > time())
			{
				$videos .= '<tr class="scheduled">'; 
			} elseif ($r['restricted'] == '1') {
				$videos .= '<tr class="private">'; 
			} else {
				$videos .= '<tr>';					
			}
			...
			if ($r['restricted'] == '1')
			{
				...
				$videos .= '<div class="pm-sprite ico-locked" rel="tooltip" align="absbottom" title="Only registered users can watch this video."></div>';
				$videos .= '</td>';
			} else {
			...
			if ($r['added'] > time())
			{
				$videos .= ' &mdash; <small>Scheduled</small>'; 
			}
			...
			if($video_status == 1){ $vid_sts_set = "<font color=\"green\"><b> Online<b/></font>";}
			else{ $vid_sts_set = "<font color=\"red\"><b> Offline<b/></font>";}
			//status
			...
			$videos .= ' <a href="#" onClick=\'promote_video_id("'. $r['uniq_id'] .'", "'. $page .'", "'. $filter .'")\' class="btn btn-mini btn-link" rel="tooltip" title="Promote video"><i class="icon-star"></i></a>';
			if ($r['trending'] == null || $r['trending'] == 0) {// unflaged as trending. 
				$videos .= ' <a href="#" data-value="0" onClick=\'trend_video_id($(this), "'. $r['uniq_id'] .'", "'. $page .'", "'. $filter .'")\' class="btn btn-mini btn-link" rel="tooltip" title="Flag as Trending Video"><i class="icon-flag"></i></a>';
			}
			else { // already flaged as trending
				$videos .= ' <a href="#" data-value="1" onClick=\'trend_video_id($(this), "'. $r['uniq_id'] .'", "'. $page .'", "'. $filter .'")\' class="btn btn-mini btn-link" rel="tooltip" title="Unflag as Trending Video"><i class="icon-fire"></i></a>';	
			}
			$videos .= '</td>';
	...
	if($count >= 1) 
	{
		...
		while($r = mysql_fetch_assoc($query)) 
		{
			...
			if ($r['source_id'] == $sources['localhost']['source_id'])
			{
				...
				$thumb_url = ($r['thumbnail'] != '') ? _URL.'/uploads/thumbs/'.$r['thumbnail'] : _URL .'/'. _ADMIN_FOLDER .'/img/no-thumbnail.jpg';
                $videos .= '
	...
	if ( ! empty($tag) && ! empty($name))
	{
		if (@preg_match($pattern, $tag))
		{
			...
			if ($tags_count == 0)
			{
				...
				{
					$table_cols = 'parent_id, tag, name, published_videos, total_videos, position, description, meta_tags';
				}
...
function a_list_vreports($r_type, $from = 0, $to = 50, $page = 1) {
	...
	if($count >= 1) {
		...
		$reports .= '<td align="center" style="text-align:center; width: 12px;"><div class="pm-sprite '. $status_img .'" id="status_'. $r['vid'] .'" alt="" rel="tooltip" title="'.$status.' <br> Last checked: '.$last_check.'"></div></td>';					
...
function show_pm_notes() {
	...
	if ($userdata['password'] == $admin_pass) 
	{
		$txt_notes[$i]['title'] = 'Enable Desired Modules';
		$txt_notes[$i]['desc']  = 'The "Article Module" and "Social Module" are disabled by default. To enable any modules, visit <a href="settings.php#t6"><strong>Settings > Available Modules</strong></a>.';
		$txt_notes[$i]['ico']  = 'gritter-ico_setup.png';
		$txt_notes[$i]['bgcolor'] = 'red';
		$i++;
	}
	...
	{
		//sitemap_last_build
		//video-sitemap_last_build
		$time_now = time();
		
		if ($time_now > ($sitemap_options['sitemap_last_build'] + (86400 * 14)) && $config['published_videos'] > $sitemap_options['total_videos'])
		{
			$txt_notes[$i]['title'] = 'Sitemap is outdated';
			$txt_notes[$i]['desc']  = 'It\'s been more than two weeks since the <strong>sitemap</strong> has been generated. Consider updating and resubmitting it to search engines. <a href="sitemap.php?type=sitemap"><strong>Update sitemap</strong></a>.';
			$txt_notes[$i]['ico']  = 'gritter-ico_info.png';
			$txt_notes[$i]['bgcolor'] = 'red';
			$i++;
		}
		
		if ($time_now > ($sitemap_options['video-sitemap_last_build'] + (86400 * 14)) && $config['published_videos'] > $sitemap_options['total_videos'])
		{
			$txt_notes[$i]['title'] = 'Video sitemap is outdated';
			$txt_notes[$i]['desc']  = 'It\'s been more than two weeks since the <strong>video sitemap</strong> has been generated. Consider updating and resubmitting it to search engines. <a href="sitemap.php?type=sitemap"><strong>Update video sitemap</strong></a>.';
			$txt_notes[$i]['ico']  = 'gritter-ico_info.png';
			$txt_notes[$i]['bgcolor'] = 'red';
			$i++;
		}
...
function a_fetch_video_sources($sort = '')
{
	...
	while($row = mysql_fetch_assoc($result))
	{
		if ($row['source_name'] == 'mp3')
		{
			...
			// silent update @since v2.2.1 // EDITME 
			if (strpos($row['embed_code'], 'jwplayer.swf') !== false)
			{
				$new_embed_code = str_replace('%%site_url%%/jwplayer.swf', '%%player_url%%', $row['embed_code']);
				$sql = "UPDATE pm_sources 
						SET embed_code = '". secure_sql($new_embed_code) ."' 
						WHERE source_name = 'mp3'";
				@mysql_query($sql);
			}

			// silent update @since v2.3.1 // EDITME 
			if ((int) $row['flv_player_support'] == 0)
			{
				$sql = "UPDATE pm_sources 
						SET flv_player_support = '1' 
						WHERE source_name = 'mp3'";
				@mysql_query($sql);
			}
		}
				...
				if (strlen($delete_ids_str) > 2)
				{
					...
					for ($i = $start; $i < $start + $inc; $i++)
					{
						...
						if ($result = @mysql_query($sql))
						{
							while ($row = mysql_fetch_assoc($result))
							{
								playlist_delete_item($row['list_id'], $video_id);
							}
							mysql_free_result($result);
						}
					}
...
function get_import_subscriptions($type = 'search', $start = 0, $limit = 0)
{
	...
	$sql = "SELECT pm_import_subscriptions.*, pm_users.username 
