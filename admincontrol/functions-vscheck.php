...
function vscheck_get_video_status($args)
{
	...
	switch ( (int) $video['source_id'])
	{
		default:
			...
			$display_message = "Sorry, Video ID '". $video['uniq_id'] ."' cannot be checked.";
			...
			else
			{
				...
				switch ($code)
				{
					...
					case 502:
						...
						$status = VS_BROKEN;
						$display_message = 'The server responded with a <code>'. $headers[0] .'</code> to your request.';
						$api_message = 'The server responded with a "'. $headers[0] .'" when tried a status check.';
					...
					case 503:
						...
						$display_message = 'The server is temporarily unavailable. Try again later.';
		...
		case 3:	//	Youtube
			...
			{
				$display_message = "Youtube API requires a public <em>API Key</em> to retrieve data.";
				break;
			}
			...
			{
				$display_message = $status['error']['message']; // api error
			}
				...
				if ($result === null)
				{
					...
					switch ($code)
					{
						...
						case 502:
							...
							$status = VS_BROKEN;
							$display_message = 'The server responded with a <code>'. $headers[0] .'</code> to your request.';
							$api_message = 'The server responded with a "'. $headers[0] .'".';
