...
$sql = "select * ".
	"from (select ".
		...
		" from pm_videos) as result ".
		" where pmv_id='".$vid."'";//.$where." order by date_added desc, added desc limit ".($pageNumber*$pageLimit).",".$pageLimit;
//echo $sql;
...
if (!$list) {
	echo "There's no result from sql.";
	exit;
}
