...
function promote_video_id(ID, PAGE, FILTER) {
	/*this.id=ID
	this.page=PAGE
	this.filter=FILTER*/
	var confirmmessage = "Are you sure you want to promote video ID #"+ID+" ?";
	/*var pmnonce = $('#_pmnonce_admin_videos_listcontrols').val();
	var pmnonce_t = $('#_pmnonce_t_admin_videos_listcontrols').val();
	var filter = $('#listing-filter').val();
	var filter_value = $('#listing-filter_value').val();*/
	
	//var goifokay = "promote.php?vid="+ this.id +"&a=1&page="+ this.page +"&filter="+ filter +"&fv="+ filter_value +"&_pmnonce="+ pmnonce + "&_pmnonce_t=" + pmnonce_t;
	
	if (confirm(confirmmessage)) {
		var http = new XMLHttpRequest();
		var url = "/admincontrol/promote.php?vid=" + ID + "&message=";
		...
		$data = array(
			'data' => 'PROMOTED:' + ID,
			'registration_ids' => '/topics/promote'
		);
...
function trend_video_id(ELEMENT, ID, PAGE, FILTER) {
	/*this.id=ID
	this.page=PAGE
	this.filter=FILTER*/
	var FLAG = ELEMENT.attr("data-value");
	console.log(ELEMENT.children());
	if (FLAG == 0) {
		var confirmmessage = "Are you sure you want to add video ID #"+ID+" to trending list?";
		var ret_message = "Successfully added to trending list!";
	}
	else {
		var confirmmessage = "Are you sure you want to remove video ID #"+ID+" from trending list?";
		var ret_message = "Successfully removed to trending list!";
	}
	
	if (confirm(confirmmessage)) {
		 var http = new XMLHttpRequest();
		 var url = "/admincontrol/trending.php?vid=" + ID + "&message=" + "&flag-trending=1" ;
		 if (FLAG == 1) {
		 	url = "/admincontrol/trending.php?vid=" + ID + "&message=" + "&flag-trending=0" ;
		 }
		 http.onreadystatechange = function() {
		 	if (http.readyState == 4 && http.status == 200) {
		  		alert(ret_message);
		 		if (FLAG == 0) {
		 			ELEMENT.children().removeClass("icon-flag");
		 			ELEMENT.children().addClass("icon-fire");
		 			ELEMENT.attr("data-value", 1);
		 		}
		 		else {
		 			ELEMENT.children().removeClass("icon-fire");
		 			ELEMENT.children().addClass("icon-flag");
	...
	if (confirm(confirm_msg)) 
	{
		...
		$.ajax({
			...
			data: {
				...
				"id": category_id,
				"type": category_type,
				"_pmnonce": '_pmnonce_admin_catmanager',
