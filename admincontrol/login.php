...
if ($_POST['Login'] == 'Login')
{
	...
	if ($user != '' && $clearpass != '')
	{
		...
		if ($count == 0) 
		{
			...
			@log_error('Failed attempt to log in to Admin Area. (Username used: <em>'.$user.'</em> / IP: <em>'.$_SERVER['REMOTE_ADDR'].'</em>)', 'Admin login', 1);
		}
		else if ($count == 1) 
		{
      ...
      <div class="span12">
        <form action="login.php" method="post" name="login" onSubmit="return checkFields();" class="form-horizontal login-form">
          ...
          <fieldset>
            ...
            <button type="submit" name="Login" value="Login" class="btn btn-blue">Sign in</button>
          </fieldset>
        </form>
