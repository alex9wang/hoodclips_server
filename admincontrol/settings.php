...
else if ($_POST['submit'] == "Save")
{
	...
	$num_fields = array('isnew_days', 'ispopular', 'comments_page', 'account_activation', 'issmtp', 'player_autoplay', 'player_autobuff', 'default_lang', 
						...
						'mod_article', 'gzip', 'bin_rating_allow_anon_voting', 'maintenance_mode', 'featured_autoplay', 'keyboard_shortcuts', 'disable_indexing', 
						...
						'auto-approve_suggested_videos', 'register_time_to_submit'
					);
...
$highlight_fields = array();	// EDITME @todo
if ($_GET['highlight'] != '')
{
...
<?php echo csrfguard_form('_admin_settings'); ?>
        ...
        <h2 class="h2-import pull-left">Settings</h2>
            ...
            <li class="<?php echo ($selected_tab_view == 't6' || $selected_tab_view == 'modules') ? 'active' : '';?>"><a href="#t6" data-toggle="tab" class="tab-pane<?php echo ($selected_tab_view == 't6' || $selected_tab_view == 'modules') ? ' active' : '';?>">Available Modules</a></li>
            ...
            <li class="<?php echo ($selected_tab_view == 't5' || $selected_tab_view == 'video-ads') ? 'active' : '';?>"><a data-toggle="tab" href="#t5" class="tab-pane<?php echo ($selected_tab_view == 't5' || $selected_tab_view == 'video-ads') ? ' active' : '';?>">Video Ads Settings</a></li>
	    	<li class="<?php echo ($selected_tab_view == 't10' || $selected_tab_view == 'comment') ? 'active' : '';?>"><a data-toggle="tab" href="#t10" class="tab-pane<?php echo ($selected_tab_view == 't10' || $selected_tab_view == 'comment') ? ' active' : '';?>">Comment Settings</a></li>
            <li class="<?php echo ($selected_tab_view == 't7' || $selected_tab_view == 'email') ? 'active' : '';?>"><a data-toggle="tab" href="#t7" class="tab-pane<?php echo ($selected_tab_view == 't7' || $selected_tab_view == 'email') ? ' active' : '';?>">E-mail Settings</a></li>
            <li class="<?php echo ($selected_tab_view == 't9' || $selected_tab_view == 'user') ? 'active' : '';?>"><a data-toggle="tab" href="#t9" class="tab-pane<?php echo ($selected_tab_view == 't9' || $selected_tab_view == 'user') ? ' active' : '';?>">User Settings</a></li>
            </ul>
        </nav>
...
<?php if ($config['mod_article'] == '0' && $config['mod_social'] == '0' && $config['firstinstall'] > ($time_now - 259200)) : // display info message only in the first 3 days ?>
	<br />
	<?php echo pm_alert_info('The "Article Module" and "Social Module" are disabled by default. To enable any available modules, see <a href="settings.php?view=modules"><strong>Available Modules</strong>.</a>'); ?>
<?php endif; ?>

<?php if ( empty($config['youtube_api_key']) ) : ?>
	<br /> 
	<?php echo pm_alert_warning('<strong>Before importing your first YouTube video...</strong> 
					<br /><br />
...
<table width="100%" border="0" cellspacing="1" cellpadding="0">
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
	  ...
	  <tr>
		  ...
		  <td><label><input name="gzip" type="radio" value="1" <?php echo ($config['gzip']==1) ? 'checked="checked"' : "";?> /> Enabled</label>
			<label><input name="gzip" type="radio" value="0" <?php echo ($config['gzip']==0) ? 'checked="checked"' : "";?> /> Disabled</label> <a href="#" rel="popover" data-placement="right" data-trigger="hover" data-content="Enable gzip compression for faster loading times."><i class="icon-info-sign"></i></a>
          </td>
	  </tr>
	  <tr>
	...
	<h2 class="sub-head-settings">Video Importing &amp; Revenue Sharing</h2>
	<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
		<tr>
			<td width="20%">
			...
			<td>
				...
				<a href="#" rel="popover" data-placement="right" data-trigger="hover" data-content="To enable Youtube video importing in the PHP Melody back-end you need an API key from your Google Developers Console. Please visit https://developers.google.com/youtube/registering_an_application for more details."><i class="icon-info-sign"></i></a> <a href="https://console.developers.google.com" target="_blank">Get key</a> <!-- // EDITME maybe link our instructional blog-post/video -->
			</td>
		</tr>
		<tr>
			<td width="20%">
			...
			<td>
				<input type="text" name="vimeo_api_token" class="input-xlarge" value="<?php echo $config['vimeo_api_token'];?>" /> <a href="#" rel="popover" data-placement="right" data-trigger="hover" data-content="To enable Vimeo video importing in the PHP Melody back-end you need an API key from Vimeo.com. Please visit https://developer.vimeo.com/api for more details."><i class="icon-info-sign"></i></a> <a href="https://developer.vimeo.com/api" target="_blank">Get key</a>
			</td>
		</tr>
		<tr>
			<td width="20%">
				Dailymotion Syndication Key
			</td>
			<td>
				<input type="text" name="dailymotion_syndication_key" class="input-xlarge" value="<?php echo $config['dailymotion_syndication_key'];?>" />  <a href="#" rel="popover" data-placement="right" data-trigger="hover" data-content="Earn money with DailyMotion's Publisher Network. For more details, visit: http://publisher.dailymotion.com/en/faqs"><i class="icon-info-sign"></i></a> <a href="http://publisher.dailymotion.com/en/backoffice/get-started" target="_blank">Get key</a>
			</td>
		</tr>
	</table>
    </div>
   ...
   <table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
	...
	if( $config['mail_server'] == 'mail.domain.com' )
		echo pm_alert_danger( "<strong>" . _SITENAME . " can't send any emails at this time.</strong> <br>Please replace the <em>mail server</em>, <em>mail port</em>, <em>mail user</em> and <em>password</em> with an actual email account.");
	?>
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables pm-tables-settings">
	 ...
	 <tr>
		<td width="20%">Auto-approve videos suggested &amp; uploaded by users</td>
		<td>
		<label><input name="auto-approve_suggested_videos" type="radio" value="1" <?php echo ($config['auto-approve_suggested_videos']==1) ? 'checked="checked"' : "";?> /> Yes</label>
		<label><input name="auto-approve_suggested_videos" type="radio" value="0" <?php echo ($config['auto-approve_suggested_videos']==0) ? 'checked="checked"' : "";?> /> No</label>
		</td>
	 </tr>
	 <tr>
