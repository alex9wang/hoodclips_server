...
if($_POST['submit'] != '')
{
	...
	$input['video_status']		= 	(int) $_POST['video_status'];
	$input['fvid_cat']		= 	 $_POST['fvid_cat'];
	$input['fvid_type']		= 	 $_POST['fvid_type'];
    	$input['fvid_sdetail']		= 	 $_POST['fvid_sdetail'];
	...
	$sql = "UPDATE pm_videos 
			SET video_title = '". $input['video_title'] ."', 
				...
				fvid_cat 	= '". $input['fvid_cat'] ."',
				fvid_type = '". $input['fvid_type'] ."',
				fvid_sdetail = '". $input['fvid_sdetail'] ."',
...
switch($action)
{
	case 1:
		...
		//@mysql_query("DELETE FROM pm_favorites WHERE uniq_id = '".$uniq_id."'");// @deprecated since v2.2
		...
		{
			if (file_exists(_VIDEOS_DIR_PATH . $video['url_flv_raw']) && strlen($video['url_flv_raw']) > 0)
			{
				unlink(_VIDEOS_DIR_PATH . $video['url_flv_raw']);
			}
		}
...
if($r['added'] > time()) 
{
	$message = pm_alert_warning('This video is <strong>scheduled</strong> to appear on your site in <strong>'. count_days($r['added'], time()) .' days</strong>.<br> <small>Change the "Publish date" below to update its schedule date ('. date("M d, Y g:i A", $r['added']) .').</small>');
}
?>
...
<select name="fvid_cat" style="width: 100%;">
<option value="">Select Video Position</option>
<option value="1" <?php if($r['fvid_cat'] == 1) echo 'selected';?>>Main Featured</option>
<option value="2" <?php if($r['fvid_cat'] == 2) echo 'selected';?>>Small 1 Featured</option>
<option value="3" <?php if($r['fvid_cat'] == 3) echo 'selected';?>>Small 2 Featured</option>
</select>
  
<select name="fvid_type" style="width: 100%;">
<option value="">Select Video Type</option>
<option value="1" <?php if($r['fvid_type'] == 1) echo 'selected';?>>Hoodclips Exclusive</option>
<option value="2" <?php if($r['fvid_type'] == 2) echo 'selected';?>>Sponsored</option>
</select>
   
<input name="fvid_sdetail" type="text" id="must" value="<?php echo $r['fvid_sdetail']; ?>" placeholder="" style="width: 99%;">    
    ...
    <div class="span3">
		...
		<h4>Thumbnail</h4>
            ...
            <?php else : ?>
            <a href="#" id="show-thumb" rel="tooltip" title="Click here to specify a custom thumbnail URL"><img src="<?php echo $r['yt_thumb']; ?>?cache_buster=<?php echo $time_now;?>" id="must" style="display:block;min-width:120px;width:100%;min-height:80px; no-repeat center center;" /></a>
            <?php endif; ?>
            <div class="">
                <div id="show-opt-thumb">
                <br />
                <input type="text" name="yt_thumb" value="<?php echo $r['yt_thumb']; ?>" class="bigger span10" placeholder="http://" /> <i class="icon-info-sign" rel="tooltip" data-position="top" title="The existing thumbnail will change after you click 'Save'."></i>
		...
		<div class="widget border-radius4 shadow-div">
			...
			<div>
				...
				<div class="pull-right">
					<span class="btn btn-mini btn-upload-widget">
						<span id="uploadSubtitlePlaceholder"></span>
					</span>
				</div>
			</div>
			...
			<div class="control-group container-fluid">
				<div class="controls row-fluid">
					<small><div id="uploadSubtitleProgress"></div></small>
					<div class="pm-swf-upload">
						<ol id="uploadSubLog"></ol>
					</div>
		...
		<label class="checkbox inline"><input type="radio" name="restricted" id="restricted" value="0" <?php if ($r['restricted'] == 0) echo 'checked="checked"'; ?> /> No</label>
            ...
            <div class="control-group">
                <label class="control-label" for="">Video Added: <span id="value-added"><strong>   (yyyy-mm-dd)       </strong></span> <a href="#" id="show-date-added"></a></label>
                <div>
                    <input type="text" name="date_added" value="<?php echo $r['date_added'];?>" size="150" style="padding: 5px 8px"/>
                </div>
            </div>
	...
	//     if ($r['source_id'] != 0) :
		...
		<h4>Video Status</h4>
            ...
            <div class="controls">
               ...
               <select name="video_status" class="span6">
	       
	        <option value="1" <?php echo ($r['video_status'] == '1') ? 'selected="selected"' : '';?>>Online</option>
	        <option value="0" <?php echo ($r['video_status'] == '0') ? 'selected="selected"' : '';?>>Offline</option>
	    </select>
		...
		<h4>Visibility Status</h4>
	        ...
	        <option value="1" <?php echo ($r['status'] == '1') ? 'selected="selected"' : '';?>>Alive</option>
	        <option value="0" <?php echo ($r['status'] == '0') ? 'selected="selected"' : '';?>>Dead</option>
	    </select>
